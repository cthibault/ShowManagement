...
namespace ShowManagement.WindowsServices.uTorrentCleanup.Service
{
    class Program
    {
        ...
        {
            TraceSourceManager.TraceSource.TraceWithDateFormat(TraceEventType.Verbose, 0, "Enter ShowManagement.WindowsServices.uTorrentCleanup.Service.Program.Main()");
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(model.ServiceName))
                {
                    ...
                    uTorrentCleanupWindowsService.SpecifiedServiceName = model.ServiceName;
                }
                ...
                if (model.Install)
                {
                    ...
                    uTorrentCleanupWindowsService.Install(args);
                ...
                if (model.Console)
                {
                    ...
                    var service = new uTorrentCleanupWindowsService();
                ...
                else if (!(model.Uninstall || model.Install))
                {
                    ...
                    TraceSourceManager.TraceSource.TraceWithDateFormat(TraceEventType.Verbose, 0, "Initializing the Service instance.");
                    var ServicesToRun = new ServiceBase[] { new uTorrentCleanupWindowsService() };
            ...
            catch (Exception ex)
            {
                ...
                TraceSourceManager.TraceSource.TraceWithDateFormat(TraceEventType.Critical, 0, "Exception caught in ShowManagement.WindowsServices.uTorrentCleanup.Service.Program.Main(): {0}.", exceptionMessage);
            ...
            TraceSourceManager.TraceSource.TraceWithDateFormat(TraceEventType.Verbose, 0, "Exit ShowManagement.WindowsServices.uTorrentCleanup.Service.Program.Main()");
        }
